 ...
 * is in that map.
 * 闭包，登记str中有的的key名字标记为true
 * expectsLowerCase 为true则不区分大小写
 */
function makeMap (
  str,
  expectsLowerCase
 ...
 * 闭包，避免重复操作，把处理过的结果存储在被保护的内存里cache，
 * 提高处理速度
 * 使用到了短路赋值  
 * hit || (cache[str] = fn(str))本来是用来返回布尔值，= 处理了赋值和返回真的双重作用
 */
function cached (fn) {
 ...
 * 大写字母转换为横岗加小写
 * tomAdnJErry 转换 tom-adm-j-e-rry
 * tom-And 转换为 tom-and
 * 存到内存
 */
