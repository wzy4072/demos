...
(function (global, factory) {
  ...
  var config = ({
    ...
    /**
     * Whether to suppress warnings.// true 取消 Vue 所有的日志与警告。
     */
    silent: false,
     ...
     * Function指定组件的渲染和观察期间未捕获错误的处理函数。这个处理函数被调用时，可获取错误信息和 Vue 实例。
     */
    errorHandler: null,
   ...
   * already exist.
   * 实例创建之后再给data对象添加属性，那么不会被双向绑定，可通过这里进行添加新属性,但是，不能添加到根实例上。
   *      Vue.set(data.message,'sex', '男');
   *      this.$set(this.$data.message,'sex','2018.05.20');
   */
   ...
   * 删除一个属性，同样，这个属性不能是跟实例下的属性
   *  this.$delete(this.$data.message,'plan')
   */
  ...
  function renderMixin(Vue) {
     ...
     * /nextTick 方法
     * @param  {Function} 
     * 在数据更新后，dom才会更新，有些需要在Dom更新后的操作可以放在回调中执行。
     */
